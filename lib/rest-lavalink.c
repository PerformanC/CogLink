...
int coglink_parsePlaylist(struct lavaInfo lavaInfo, struct httpRequest req, struct lavaParsedPlaylist **playlistStruct) {
  ...
  strcpy(playlist->selectedTrack, SelectedTrack);

  if (lavaInfo.debugging->allDebugging || lavaInfo.debugging->memoryDebugging) log_debug("[coglink:memory-strcpy] Copied %d bytes to playlist structure.", sizeof(Name) + sizeof(SelectedTrack) + 2);
...
int coglink_parseError(struct lavaInfo lavaInfo, struct httpRequest req, struct lavaParsedError **errorStruct) {
  ...
  if (lavaInfo.debugging->allDebugging || lavaInfo.debugging->memoryDebugging) log_debug("[coglink:memory-strcpy] Copied %d bytes to error structure.", sizeof(Message) + sizeof(Severity) + 2);
